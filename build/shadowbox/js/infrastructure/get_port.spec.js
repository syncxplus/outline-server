...
const get_port = require("./get_port");
...
describe('getRandomUnusedPort', () => {
    ...
    it('Rejects if it cannot find an unused port', (done) => {
        get_port
            .getRandomUnusedPort(new Set(), get_port.getRandomPortOver1023, (port) => Promise.resolve(true)) // always return port in use
            .catch(done);
    });
    it('Does not pick from reserved ports', (done) => {
        ...
        const MAX_RETRIES = 1;
        get_port
            .getRandomUnusedPort(new Set([RESERVED_PORT]), () => RESERVED_PORT, (port) => Promise.resolve(false), MAX_RETRIES)
            .catch(done);
    });
});
