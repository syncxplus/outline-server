...
describe('ServerAccessKeyRepository', () => {
    ...
    it('Repos created with an existing file restore access keys', (done) => {
        const config = new json_config_1.InMemoryConfig({ accessKeys: [], nextId: 0 });
        const repo1 = new server_access_key_1.ServerAccessKeyRepository('hostname', config, new mocks_1.MockShadowsocksServer());
        ...
        Promise.all([repo1.createNewAccessKey(), repo1.createNewAccessKey()]).then(() => {
            ...
            const repo2 = new server_access_key_1.ServerAccessKeyRepository('hostname', config, new mocks_1.MockShadowsocksServer());
...
function countAccessKeys(repo) {
    return iterToArray(repo.listAccessKeys()).length;
}
function createRepo() {
    const config = new json_config_1.InMemoryConfig({ accessKeys: [], nextId: 0 });
    const server = new mocks_1.MockShadowsocksServer();
    return new server_access_key_1.ServerAccessKeyRepository('hostname', config, server);
}
