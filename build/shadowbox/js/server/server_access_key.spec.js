...
describe('ServerAccessKeyRepository', () => {
    it('Repos with non-existent files are created with no access keys', (done) => {
        createRepo(new mocks_1.InMemoryFile(false)).then((repo) => {
    ...
    it('Repos created with an existing file restore access keys', (done) => {
        const accessKeyConfigFile = new mocks_1.InMemoryFile(false);
...
function createRepo(inMemoryFile) {
    return server_access_key_1.createServerAccessKeyRepository('hostname', inMemoryFile, new mocks_1.MockShadowsocksServer(), null, null);
}
