...
const shared_metrics = require("./shared_metrics");
...
describe('getHourlyServerMetricsReport', () => {
    it('Converts IP addresses to country codes', (done) => {
        ...
        shared_metrics
            .getHourlyServerMetricsReport(SERVER_ID, START_DATETIME, END_DATETIME, lastHourUserStats, new HardcodedIpLocationService())
            .then((report) => {
    ...
    it('Filters sanctioned countries from userReports', (done) => {
        const lastHourUserStats = new Map();
        lastHourUserStats.set(USER_ID_1, getPerUserStats([IP_ADDRESS_IN_NORTH_KOREA, IP_ADDRESS_IN_US_1]));
        lastHourUserStats.set(USER_ID_2, getPerUserStats([IP_ADDRESS_IN_US_1]));
        shared_metrics
            .getHourlyServerMetricsReport(SERVER_ID, START_DATETIME, END_DATETIME, lastHourUserStats, new HardcodedIpLocationService())
            .then((report) => {
    ...
    it('Removes userReports that contain only sanctioned countries', (done) => {
        const lastHourUserStats = new Map();
        lastHourUserStats.set(USER_ID_1, getPerUserStats([IP_ADDRESS_IN_NORTH_KOREA, IP_ADDRESS_IN_CUBA]));
        lastHourUserStats.set(USER_ID_2, getPerUserStats([IP_ADDRESS_IN_US_1]));
        shared_metrics
            .getHourlyServerMetricsReport(SERVER_ID, START_DATETIME, END_DATETIME, lastHourUserStats, new HardcodedIpLocationService())
            .then((report) => {
    ...
    it('Does not propagate location service connection errors', (done) => {
        ...
        shared_metrics
            .getHourlyServerMetricsReport(SERVER_ID, START_DATETIME, END_DATETIME, lastHourUserStats, new FailConnectionIpLocationService())
            .then((report) => {
            expect(report.userReports.length).toEqual(1);
            expect(report.userReports[0].countries.length).toEqual(1);
    ...
    it('Does not propagate location service promise rejection', (done) => {
        ...
        return shared_metrics
            .getHourlyServerMetricsReport(SERVER_ID, START_DATETIME, END_DATETIME, lastHourUserStats, new AlwaysRejectIpLocationService())
            .then((report) => {
            expect(report.userReports.length).toEqual(1);
            expect(report.userReports[0].countries.length).toEqual(1);
            expect(report.userReports[0].countries[0]).toEqual('ERROR');
...
class FailConnectionIpLocationService {
    countryForIp(ipAddress) {
        const countryPromise = new Promise((fulfill, reject) => {
            https
                .get('https://0.0.0.0', (response) => {
                response.on('end', () => {
...
class AlwaysRejectIpLocationService {
    countryForIp(ipAddress) {
        return Promise.reject(new Error(`This IpLocationService always rejects. ipAddress: ${ipAddress}`));
    }
}
