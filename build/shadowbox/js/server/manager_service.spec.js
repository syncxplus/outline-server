...
describe('ShadowsocksManagerService', () => {
    ...
    describe('getServer', () => {
        it('Return default name if name is absent', (done) => {
            const repo = new mocks_1.MockAccessKeyRepository();
            const serverConfig = new json_config_1.InMemoryConfig({});
            const service = new manager_service_1.ShadowsocksManagerService('default name', serverConfig, repo, null);
            service.getServer({ params: {} }, {
                send: (httpCode, data) => {
        ...
        it('Return saved name', (done) => {
            const repo = new mocks_1.MockAccessKeyRepository();
            const serverConfig = new json_config_1.InMemoryConfig({ name: 'Server' });
            const service = new manager_service_1.ShadowsocksManagerService('default name', serverConfig, repo, null);
            service.getServer({ params: {} }, {
                send: (httpCode, data) => {
                    expect(httpCode).toEqual(200);
    ...
    describe('renameServer', () => {
        it('Rename changes the server name', (done) => {
            ...
            const serverConfig = new json_config_1.InMemoryConfig({});
            const service = new manager_service_1.ShadowsocksManagerService('default name', serverConfig, repo, null);
            service.renameServer({ params: { name: 'new name' } }, {
                send: (httpCode, _) => {
    ...
    describe('listAccessKeys', () => {
        it('lists access keys in order', (done) => {
            const repo = new mocks_1.MockAccessKeyRepository();
            const service = new manager_service_1.ShadowsocksManagerService('default name', null, repo, null);
    ...
    describe('getShareMetrics', () => {
        it('Returns value from config', (done) => {
            ...
            const service = new manager_service_1.ShadowsocksManagerService('default name', serverConfig, null, null);
            service.getShareMetrics({ params: {} }, {
                send: (httpCode, data) => {
                    expect(httpCode).toEqual(200);
    ...
    describe('setShareMetrics', () => {
        it('Sets value in the config', (done) => {
            const serverConfig = new json_config_1.InMemoryConfig({ metricsEnabled: false });
            const service = new manager_service_1.ShadowsocksManagerService('default name', serverConfig, null, null);
            service.setShareMetrics({ params: { metricsEnabled: true } }, {
                send: (httpCode, _) => {
                    expect(httpCode).toEqual(204);
...
function getFirstAccessKey(repo) {
    return repo.listAccessKeys().next().value;
}
function createNewAccessKeyWithName(repo, name) {
