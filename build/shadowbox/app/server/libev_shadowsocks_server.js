...
class LibevShadowsocksServer {
    constructor(publicAddress, metricsSocket, ipLocation, usageWriter, verbose) {
        this.publicAddress = publicAddress;
        this.metricsSocket = metricsSocket;
        this.verbose = verbose;
        this.portId = new Map();
        this.portInboundBytes = new Map();
        metricsSocket.on('message', (buf) => {
            let metricsMessage;
            try {
    ...
    startInstance(id, portNumber, password, encryptionMethod) {
        ...
        return Promise.resolve(new LibevShadowsocksServerInstance(childProcess, portNumber, password, encryptionMethod, accessUrl));
    }
}
exports.LibevShadowsocksServer = LibevShadowsocksServer;
class LibevShadowsocksServerInstance {
    constructor(childProcess, portNumber, password, encryptionMethod, accessUrl) {
...
function getConnectedClientIPAddresses(portNumber) {
    ...
    return execCmd(lsofCommand).then((output) => {
        return output.split('\n');
    });
}
